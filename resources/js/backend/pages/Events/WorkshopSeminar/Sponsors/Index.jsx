...
import {
    ...
    fetchAllSponsors,
    updateSponsor
} from "@/featurs/WorkshopSponsors/WorkshopSponsorSlice.js";
...
function Index(props) {
    const {id} = useParams();
    const {isLoading, sponsors} = useSelector((state) => state.WorkshopSponsorReducer);
    ...
    const [title, setTitle] = useState("Add New Sponsor");
    ...
    const requestHandler = (e) => {
        ...
        if (cName) {
            ...
            if (!isEdit) {
                dispatch(createSponsors(formData))
            } else {
                let data = {
                    dataset: formData,
                    item: selectedId
    ...
    const handelOrganizerEdit = (id) => {
        setSelectedId(id)
        let dayMeta = sponsors.filter((sponsor) => sponsor.id === id)
        dayMeta = dayMeta[0]
